...
import { ChevronDown, ChevronLeft, ChevronRight, MoreHorizontal, Plus, Trash } from 'lucide-react';
...
export default function Item({ label, id, level, onExpand, expanded }: ItemProps) {
    const { user } = useUser();
    ...
    return (
        <div style={{ paddingLeft: level ? `${level * 12 + 12}px` : "12px" }} className={"group min-h-[27px] text-sm py-1 pr-3 w-full hover:bg-primary/5 flex items-center text-muted-foreground font-medium"}>
            {!!id && (
